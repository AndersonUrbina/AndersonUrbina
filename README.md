<h1>Hi, I'm Anderson! <br/><a href="https://www.morainepark.edu/about-mptc/deans-list/#:~:text=Unke-,Anderson,Urbina,-Breleigh">Moraine Park Technical College - Future IT Software Development</a></h1>

<h2>👨‍💻 Software Development Projects:</h2>

- <b>IT Support Ticket Manager (C#)</b>
  - [Course Project - Console application to manage IT service tickets.](https://github.com/AndersonUrbina/IT-Support-Ticket-Manager)
- <b>Product Discount Calculator (PHP)</b>
  - [Program to calculate discount of a product](https://github.com/joshmadakor1/4chan-Image-Analysis-Middleware-C964)
- <b>Website based on HTML, CSS, Js, PHP</b>
  - [IT includes form validation and email message to confirm successful submission.](https://github.com/joshmadakor1/Sentinel-Lab)
- <b>def-function-app (Python)</b>
  - [It calculates the gross income, net pay, and deductions based on hourly wage and weekly hours worked](https://github.com/joshmadakor1/EncrypterPOC)
- <b>Software Development LifeCycle (SDLC)</b>
  - [Feasibility Study based on Work Order App scenario](https://github.com/joshmadakor1/Package-Delivery-Pathfinding-Algorithm)
  - [Context Flow Diagram (CFD)- Work Order App]
  - [Level 0 Data Flow Diagram (DFD) - Work Order App]

<h2>Currently Learning</h2>

<strong>Evolvivo Article Publishing Platform | HTML, CSS, JavaScript, MySQL, REST APIs</strong><br>
<strong>🚧 Status:</strong> In Process
- I am currently developing a personal full-stack web application that serves as an article publishing platform. The project integrates a MySQL database with a JavaScript-based server to build RESTful APIs that retrieve and manage article data. These APIs dynamically deliver content to the front-end, where the articles are rendered for users. This is my first full-stack project, focused on implementing backend-to-frontend communication through basic API connections and database integration. 

<h2> 🤳 Connect with me:</h2>

[<img align="left" alt="JoshMadakor | Twitter" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v3/icons/twitter.svg" />][twitter]
[<img align="left" alt="JoshMadakor | LinkedIn" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v3/icons/linkedin.svg" />][linkedin]
[<img align="left" alt="JoshMadakor | Instagram" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v3/icons/instagram.svg" />][instagram]

[twitter]: https://x.com/andersondurbina
[instagram]: https://www.instagram.com/andersondurbina/
[linkedin]: https://www.linkedin.com/in/anderson-urbina-8b6810382/

<!--
**AndersonUrbina/AndersonUrbina** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.
-->
